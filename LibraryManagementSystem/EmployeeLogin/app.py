from flask import Flask, flash, redirect, url_for, render_template, session, logging, request
...
class RegisterForm(Form):
    employeeName = StringField("Name", [validators.Length(min=1, max=100)])
    employeeUsername = StringField(
        'employee Username', [validators.Length(min=1, max=100)])
...
def register():
        ...
        if request.method == 'POST' and form.validate():
            ...
            employeeUsername = form.employeeUsername.data
            ...
            # Executing Query
            cur.execute("INSERT INTO employee( employeeName, employeeUsername, password) VALUES(%s, %s, %s)",
                        (employeeName, employeeUsername, password))
...
def login():
    if request.method == 'POST':
        ...
        result = cur.execute(
            "SELECT * FROM employee WHERE employeeUsername = %s", [employeeUsername])
        ...
        if result > 0:
            ...
            if sha256_crypt.verify(password_candidate, password):
                ...
                session['employeeUsername'] = employeeUsername
...
class IssueForm(Form):
    ...
    customerUsername = StringField(
        "customer ID number", [validators.Length(min=1)])
    employeeUsername = StringField('Enter your ID to authenticate', [
...
def issue_books(bookName):
    ...
    if request.method == 'POST' and form.validate():
        customer_id = form.customerUsername.data
        employee_id = form.employeeUsername.data
        bookName = form.bookName.data

        # Execute
        cur.execute("INSERT INTO transactions( customerUsername, employeeUsername, bookName, book_id) VALUES(%s, %s, %s, %s)",
                    (customer_id, session['employeeUsername'], bookName, book['book_id']))  # , session['username']))
        cur.execute(
...
def return_books():
    ...
    if result > 0:
        ...
        if request.method == 'POST' and form.validate():
            ...
            cur = mysql.connection.cursor()
            result = cur.execute("select book_id from transactions where customerUsername= "+str(
                customer_id)+" and bookName= '"+str(book_name)+"' ")
            data = cur.fetchone()
            if result > 0:
                ...
                cur.execute("update transactions set Done = 1  where book_id = " +
                            str(book_id)+" and customerUsername= "+str(customer_id)+" ")

                mysql.connection.commit()

                cur.execute("select returnDate from transactions where customerUsername = " +
                            str(customer_id)+" and book_id= "+str(book_id)+" ")
                data = cur.fetchone()
            ...
            - datetime.datetime.strptime(returndate, datetimeFormat)
                    amount_to_be_added_to_due = (diff.days)*10

                    cur.execute("update transactions set due=due+ "+str(
                        amount_to_be_added_to_due)+" customerUsername= "+str(customer_id)+"  ")
                    mysql.connection.commit()
...
class GetUsernameForm(Form):
    customerUsername = StringField(
        "customer ID number", [validators.Length(min=1)])
    amountpaid = StringField("customer ID number")
...
def check_due():
    ...
    # Execute
    result = cur.execute(
        "SELECT customerUsername, due  FROM transactions where due > 0 GROUP BY customerusername,due")

    books = cur.fetchall()

    if result > 0:
        return render_template('check_due.html', books=books)
    else:
        msg = 'No outstanding dues'
        return render_template('check_due.html', msg=msg)
...
@app.route('/pay_due', methods=['GET', 'POST'])
@is_logged_in
def pay_due():
    ...
    if request.method == 'POST' and form.validate():
        ...
        cur = mysql.connection.cursor()
        cur.execute(
            "select due from transactions where customerUsername="+str(customer_id)+"  ")
        data = cur.fetchone()
        amountpaid = form.amountpaid.data
        if amountpaid and int(data['due']) > 0:

            originaldue = int(data['due'])
            newdue = 0
            newdue = originaldue-int(amountpaid)
            print(newdue)
            cur.execute("update transactions set due="+str(newdue) +
                        " where customerUsername="+str(customer_id)+" ")

            mysql.connection.commit()

            flash('Amount was paid', 'success')

    return render_template('pay_due.html', form=form, data=data, newdue=newdue)
...
def analyse():
    cur = mysql.connection.cursor()
    cur.execute("select customerUsername,count(*) as num from transactions group by customerUsername,due order by due  desc, num desc limit 5")
    data = cur.fetchall()
    print data
    mysql.connection.commit()
